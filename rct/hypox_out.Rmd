...
These outcomes are only evaluated in the intervention arms. 
...
###young infants
#frequencies
hypox_yi <- as_label(table(d0$country[which(d0$yg_infant==1 & d0$intervention%in%c(1,2))], d0$hypox_sum[which(d0$yg_infant==1 & d0$intervention%in%c(1,2))]))
hypox_yi <- rbind(margin.table(hypox_yi, 2), hypox_yi)
  
#totals
num_yi <- as_label(table(d0$country[which(d0$yg_infant==1 & d0$intervention%in%c(1,2))]))
num_yi <- c(margin.table(num_yi), num_yi)
  
###older children
#frequencies
hypox_ch <- as_label(table(d0$country[which(d0$yg_infant==0 & d0$intervention%in%c(1,2))], d0$hypox_sum[which(d0$yg_infant==0 & d0$intervention%in%c(1,2))]))
hypox_ch <- rbind(margin.table(hypox_ch, 2), hypox_ch)
  
#totals
num_ch <- as_label(table(d0$country[which(d0$yg_infant==0 & d0$intervention%in%c(1,2))]))
...
for(j in 1:nrow(hypox_yi)){
  ...
  #add row names
  hypox_summ <- cbind(c("Under 2 months of age", "Combined", "India", "Tanzania", "2-59 months of age", "Combined", "India", "Tanzania"), hypox_summ)
...
### SpO2 values distribution - Intervention arms combined

```{r SPO2 INTERVENTION ARMS, fig.height = 8, fig.width=6}

#save graph overall and by country
gr <- bar_graph_3b(d0[which(d0$intervention%in%c(1,2)),], "hypox_sum", levels(as_label(d0$hypox_sum)), "Combined")
gr_in <- bar_graph_3b(d0[which(d0$intervention%in%c(1,2) & d0$country=="in"),], "hypox_sum", levels(as_label(d0$hypox_sum)), "India")
gr_tz <- bar_graph_3b(d0[which(d0$intervention%in%c(1,2) & d0$country=="tz"),], "hypox_sum", levels(as_label(d0$hypox_sum)), "Tanzania")
...
#arrange
grid.arrange(gr, gr_in, gr_tz, ncol=1, top="SpO2", legend)

```


### SpO2 values distribution - Intervention arms separately

```{r SPO2 INTERVENTION ARMS SEPARATELY, fig.height = 8, fig.width=6}

#save graph overall and by country
gr <- bar_graph_3b(d0[which(d0$intervention==1),], "hypox_sum", levels(as_label(d0$hypox_sum)), "Combined-PO")
gr_in <- bar_graph_3b(d0[which(d0$intervention==1 & d0$country=="in"),], "hypox_sum", levels(as_label(d0$hypox_sum)), "India-PO")
gr_tz <- bar_graph_3b(d0[which(d0$intervention==1 & d0$country=="tz"),], "hypox_sum", levels(as_label(d0$hypox_sum)), "Tanzania-PO")
gr_tz2 <- bar_graph_3b(d0[which(d0$intervention==2 & d0$country=="tz"),], "hypox_sum", levels(as_label(d0$hypox_sum)), "Tanzania-PO+CDSA")
...
#arrange
grid.arrange(gr, gr_in, ncol=1, top="", legend)
grid.arrange(gr_tz, gr_tz2, ncol=1, top="", legend)
...
tab_hypox <- sum.tab.cat(d0, "hypox_cat", "SpO2", var2=NULL, "intervention")
...
tab_hypox_sc <- sum.tab.cat(d0, "hypox_cat1", "SpO2", var2="sev_comp2", "intervention")
...
For this outcome, severe hypoxaemia is defined according to country's specific cut-off (SpO2<90% for both India and Tanzania). 
...
tab_hypox_sev_hyp <- sum.tab(d0, "sev_hyp", "intervention")
...
For this outcome, severe hypoxaemia is defined according to country's specific cut-off (SpO2<90% for both India and Tanzania). Proportions are presented using both all children recruited and only children with hypoxaemia as denominator.
...
tab_hypox_oxy <- sum.tab(d0, "hypo_o2_ref", "intervention")
tab_hypox_oxy.1 <- sum.tab(d0[which(d0$hypox_country==3),], "hypo_o2_ref", "intervention")
...
d0$hypox_tmp <- 0
d0$hypox_tmp[which(d0$hypox_country==3)] <- 1
d0$hypox_tmp <- as.factor(d0$hypox_tmp)


#yi
hypox_summ_tab_yi <- rbind(onerow.sum.tab(as_label(d0[which(d0$yg_infant==1),]), vars="severe_diag", vars_name="Children with severe diagnosis", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==1 & d0$severe_diag==1),]), vars="hypox_tmp", vars_name="Children with severe diagnosis with hypoxaemia", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==1 & d0$severe_diag==1 & d0$hypox_tmp==1),]), vars="urg_ref", vars_name="Children with severe diagnosis with hypoxaemia referred", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==1 & d0$severe_diag==1 & d0$hypox_tmp==1 & d0$urg_ref==1),]), vars="hos_flag", vars_name="Children with severe diagnosis with hypoxaemia admitted to hospital", by="intervention"))

hypox_summ_tab_yi[which(hypox_summ_tab_yi[,2]!=""), 2] <- "-"

#ch
hypox_summ_tab_ch <- rbind(onerow.sum.tab(as_label(d0[which(d0$yg_infant==0),]), vars="severe_diag", vars_name="Children with severe diagnosis", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==0 & d0$severe_diag==1),]), vars="hypox_tmp", vars_name="Children with severe diagnosis with hypoxaemia", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==0 & d0$severe_diag==1 & d0$hypox_tmp==1),]), vars="urg_ref", vars_name="Children with severe diagnosis with hypoxaemia referred", by="intervention"), onerow.sum.tab(as_label(d0[which(d0$yg_infant==0 & d0$severe_diag==1 & d0$hypox_tmp==1 & d0$urg_ref==1),]), vars="hos_flag", vars_name="Children with severe diagnosis with hypoxaemia admitted to hospital", by="intervention"))
