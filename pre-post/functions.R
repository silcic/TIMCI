...
onerow.sum.tab <- function(x, vars, vars_name, by, med.c=T, mean.c=F, ci=F, dig=F){
  ...
  for(k in 1:length(vars)){
    ...
    if(class(x[,vars[k]])=="integer" | class(x[,vars[k]])=="numeric"){
      ...
      if(med.c==T){
        sum.tab <- rbind(sum.tab, rbind(paste(formatC(round(med.tot, dig+1), format="f", dig+1), " (", formatC(round(q1.tot, dig+2), format="f", dig+2), ", ", formatC(round(q3.tot, dig+2), format="f", dig+2), "), ", n.tot, sep=""), paste(formatC(round(med_ke, dig+1), format="f", dig+1), " (", formatC(round(q1_ke, dig+2), format="f", dig+2), ", ", formatC(round(q3_ke, dig+2), format="f", dig+2), "), ", n.ke, sep=""), paste(formatC(round(med_se, dig+1), format="f", dig+1), " (", formatC(round(q1_se, dig+2), format="f", dig+2), ", ", formatC(round(q3_se, dig+2), format="f", dig+2), "), ", n.se, sep=""))) #median (IQR)
      }
      if(mean.c==T){
        sum.tab <- rbind(sum.tab, rbind(paste(formatC(round(mean.tot, dig+1), format="f", dig+1), " (", formatC(round(sd.tot, dig+2), format="f", dig+2), "), ", n.tot, sep=""), paste(formatC(round(mean_ke, dig+1), format="f", dig+1), " (", formatC(round(sd_ke, dig+2), format="f", dig+2), "), ", n.ke, sep=""),  paste(formatC(round(mean_se, dig+1), format="f", dig+1), " (", formatC(round(sd_se, dig+2), format="f", dig+2), "), ", n.se, sep=""))) #mean (SD)
      }
      
      #add row names
      sum.tab <- as.data.frame(cbind(c(vars_name[k], "Combined", "Kenya", "Senegal"), sum.tab))
      names(sum.tab) <- c("", "pre", "post") 
    ...
    if(class(x[,vars[k]])!="integer" & class(x[,vars[k]])!="numeric"){
      ...
      #add row names
      sum.tab <- cbind(c(vars_name[k], "Combined", "Kenya", "Senegal"), sum.tab)
      names(sum.tab) <- c("", "pre", "post") 
  ...
  #add col names
  names(tab) <- c("", "Pre-intervention", "Post-intervention")
  tab
...
sum.tab <- function(x, var, by, ci=F){
  ...
  #combine data for table 
  sum.tab_yi <- data.frame("pre"="", "post"="")
  sum.tab_ch <- data.frame("pre"="", "post"="")
  ...
  #add row names
  sum.tab <- cbind(c("Under 2 months of age", "Combined", "Kenya", "Senegal", "2-59 months of age", "Combined", "Kenya", "Senegal"), sum.tab)
  
  #replace NaN% with "-"
  sum.tab[sum.tab=="NaN% (0/0)"] <- "- (0/0)"
  
  #add col names
  names(sum.tab) <- c("", "Pre-intervention", "Post-intervention")
  sum.tab
...
sum.tab.cat <- function(x, var, var_name, var2, by){
  ...
  for(j in 1:ncol(fr_yi_co)){
    ...
    #combine all summaries
    temp <- c("", paste(perc_yi_co, fr_n_yi_co), paste(perc_yi_ke, fr_n_yi_ke), paste(perc_yi_se, fr_n_yi_se), "", paste(perc_ch_co, fr_n_ch_co), paste(perc_ch_ke, fr_n_ch_ke), paste(perc_ch_se, fr_n_ch_se))
    
    sum.tab <- cbind(sum.tab, temp)
  }
  
  #add row names
  sum.tab <- cbind(c("Under 2 months of age", "Combined", rep("", (nlevels(x[,var])-1)), "Kenya", rep("", (nlevels(x[,var])-1)), "Senegal", rep("", (nlevels(x[,var])-1)), "2-59 months of age", "Combined", rep("", (nlevels(x[,var])-1)), "Kenya", rep("", (nlevels(x[,var])-1)), "Senegal", rep("", (nlevels(x[,var])-1))), c(rep(c("", rep(levels(as_label(x[,var])),3)),2)), sum.tab)
  ...
  #add col names
  names(sum.tab) <- c("", var_name, "Pre-intervention", "Post-intervention")
  sum.tab
...
bar_graph_perc <- function(x, spo2_var, fid_list,  fid_list_names, pseudo.dt_post){
  ...
  gr <- ggplot(dt, aes(y=Percentage, x=Time)) + geom_bar( stat="identity", fill="lightblue") +
    scale_x_discrete(name="Weeks", labels=pseudo.dt_post) +  #customize x axis
...
bar_graph_2b <- function(x, x1, fid_list, fid_list_names, pseudo.dt_post){
  
  #children recruited per week
  x$week <- factor(x$week, levels=seq(1,length(pseudo.dt_post),1))
  tab_d0 <- table(x$week)
  
  #cdsa records per week
  x1$week <- factor(x1$week, levels=seq(1,length(pseudo.dt_post),1))
...
summMod <- function(mod, lim=F){
  
  co.tab <- as.data.frame(summary(mod)$coef)
  if(any(rownames(co.tab)=="factor(pre_post)1")==T){co.tab <- co.tab[row.names(co.tab)%in%c("factor(pre_post)1"),]}
  if(any(rownames(co.tab)=="factor(pre_post_sa1)1")==T){co.tab <- co.tab[row.names(co.tab)%in%c("factor(pre_post_sa1)1"),]}
  if(any(rownames(co.tab)=="factor(pre_post_sa2)1")==T){co.tab <- co.tab[row.names(co.tab)%in%c("factor(pre_post_sa2)1"),]}
  ...
  #RD and 95% CI
  require(emmeans)
  if(lim==T & any(rownames(co.tab)=="factor(pre_post)1")==T){mar <- emmeans(mod, specs = "pre_post", regrid = "response", rg.limit=48000)}; if(lim==F & any(rownames(co.tab)=="factor(pre_post)1")==T){mar <- emmeans(mod, specs = "pre_post", regrid = "response")}
  if(lim==T & any(rownames(co.tab)=="factor(pre_post_sa1)1")==T){mar <- emmeans(mod, specs = "pre_post_sa1", regrid = "response", rg.limit=48000)}; if(lim==F & any(rownames(co.tab)=="factor(pre_post_sa1)1")==T){mar <- emmeans(mod, specs = "pre_post_sa1", regrid = "response")}
  if(lim==T & any(rownames(co.tab)=="factor(pre_post_sa2)1")==T){mar <- emmeans(mod, specs = "pre_post_sa2", regrid = "response", rg.limit=48000)}; if(lim==F & any(rownames(co.tab)=="factor(pre_post_sa2)1")==T){mar <- emmeans(mod, specs = "pre_post_sa2", regrid = "response")}
